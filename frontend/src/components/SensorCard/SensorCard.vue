<template>
  ...
  <v-container class="pa-1 mt-n1" fluid style="width: 100%">
    <v-card color="grey-darken-3" elevation="5" max-width="500" class="pa-0">
      <template v-slot:title>
        <v-spacer class="mt-3"></v-spacer>
        <v-icon icon="mdi mdi-leak" size="30"></v-icon>
        <text class="text-subtitle-3 ml-2 pa-0">Sensors</text>
      ...
      <v-container class="pa-1 mt-n3 ml-3">
        ...
        <v-row v-for="(sensor, index) in sensorsState" :key="sensor.SensorID" no-gutters class="mt-n1">
          ...
          <v-col>
            <v-icon
              v-if="sensor.Type != 'temp'"
              icon="mdi mdi-circle-medium"
              size="35"
              v-bind:color="sensor.sensorData ? 'green-accent-3' : 'grey-darken-1'"
              class="mt-n1 mb-n1"
            ></v-icon>
